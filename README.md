...
- `01_feature_engineering.ipynb`  
  Формирование признаков на уровне «клиент–период–категория».  
  Ноутбук читает сырые транзакции и train-файлы, с помощью DuckDB считает 7 агрегированных признаков за 30 дней по каждой категории (суммы, количества, доли, “свежесть” активности, время суток) и сохраняет результат в `train_feat7_all.parquet` / `test_feat7_all.parquet`.

- `02_catboost_ovr_training.ipynb`  
  Обучение one-vs-rest ансамбля из 32 моделей CatBoost, подбор порогов и формирование сабмита.  
  Ноутбук объединяет пер-категорийные признаки с клиентским профилем (возраст, сегмент, день недели), обучает CatBoost для каждой категории, подбирает категорийно-зависимые пороги по валидации (prevalence-init + уточнение по взвешенному Hamming), сохраняет модели и метрики, строит heatmap важности признаков и формирует итоговый `submission_best.csv`.
...
5. **Формирование итогового предсказания.**  

6. **Анализ важности признаков и предсказания моделей.**  
...
CatBoost для каждой категории возвращает **вероятность** `p` того, что категория должна быть включена в ответ.  
...
1. вокруг `T0` задаётся небольшой диапазон значений (`T0 ± 0.25`);
2. этот диапазон разбивается на 15 точек (равномерная сетка);
...
(без учёта соревновательных весов):
...
**Итоговое качество модели: 0.49014** на private leaderboard (13-е место).
